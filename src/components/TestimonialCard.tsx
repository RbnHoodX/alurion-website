    ...
    delay: delay * 0.1
      ...
      <Card className="bg-white border-none rounded-3xl overflow-hidden h-full shadow-[0_4px_20px_rgba(0,0,0,0.08)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)] transition-all duration-300">
        <CardContent className="p-8 flex flex-col h-full">
          {/* Stars at the top of quote */}
          <div className="flex mb-6">
            {[...Array(5)].map((_, i) => <svg key={i} className="w-5 h-5 text-alurion-accent" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="relative mb-8">
            <svg className="absolute top-0 left-0 transform -translate-y-4 w-8 h-8 text-alurion-accent opacity-20" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="mt-auto flex items-center">
            <Avatar className="h-12 w-12 border-2 border-alurion-secondary/20">
              {image ? <AvatarImage src={image} alt={author} /> : <AvatarFallback className="bg-alurion-primary text-white">
                  {initials}
                </AvatarFallback>}
            </Avatar>
            <div className="ml-4">
              <h3 className="font-medium text-alurion-primary text-left">{author}</h3>
              <p className="text-sm text-gray-500 text-left">{position}</p>
