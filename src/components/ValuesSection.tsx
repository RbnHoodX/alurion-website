...
const ValueCard = ({
  ...
  return <div className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-all">
      <h3 className="text-xl font-bold mb-3 text-alurion-primary">{title}</h3>
...
const ValuesSection = () => {
  ...
  return <section id="values" className="section pattern-bg-dark">
      <div className="absolute inset-0 bg-alurion-primary/40"></div>
      <div className="container mx-auto relative z-10">
        <h2 className="section-title">Our Values</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {values.map((value, index) => <ValueCard key={index} title={value.title} description={value.description} />)}
        </div>
      </div>
    </section>;
};
